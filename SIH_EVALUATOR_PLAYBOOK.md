...
| Critical | Reminder dispatch uses service-role privileges without checking caller authorization; middleware does not protect this API prefix. | `app/api/reminders/dispatch/route.ts`, `proxy.ts` | Authenticate a trusted scheduler, reject other callers, verify write results |
...
| High | ASHA triage result buttons point to patient-only routes and do not preserve an authorized assisted workflow. | `components/triage/triage-wizard.tsx`, `proxy.ts` | Role-aware assisted booking/emergency actions with patient scope checks |
